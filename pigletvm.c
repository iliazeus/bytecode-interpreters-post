...
#define TOP()                                  \
    (acc)
#define NEXT_HANDLER(code, stack_top, acc)                      \
    (((code)++), (code)->handler((code), (stack_top), (acc)))
#define END_TRACE(code, stack_top, acc)              \
    { vm_trace.stack_top = (stack_top); vm_trace.acc = (acc); return; }
#define ARG_AT_PC(bytecode, pc)                                         \
    (((uint64_t)(bytecode)[(pc) + 1] << 8) + (bytecode)[(pc) + 2])

typedef struct scode scode;

typedef void trace_op_handler(scode *code, uint64_t *stack_top, uint64_t acc);
...
static void op_abort_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    (void) code;
    ...
    END_TRACE(code, stack_top, acc);
}

static void op_pushi_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    PUSH(code->arg);

    NEXT_HANDLER(code, stack_top, acc);
}

static void op_loadi_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
...
static void op_loadaddi_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint64_t addr = code->arg;
    uint64_t val = vm_trace.memory[addr];
    TOP() += val;
...
static void op_storei_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
...
static void op_load_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint16_t addr = POP();
    uint64_t val = vm_trace.memory[addr];
    PUSH(val);
...
static void op_store_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint64_t val = POP();
    uint16_t addr = POP();
    vm_trace.memory[addr] = val;
...
static void op_dup_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    PUSH(TOP());
...
static void op_discard_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    (void) POP();
...
static void op_add_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint64_t arg_right = POP();
    TOP() += arg_right;
...
static void op_addi_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
...
static void op_sub_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint64_t arg_right = POP();
    TOP() -= arg_right;
...
static void op_div_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint64_t arg_right = POP();
    /* Don't forget to handle the div by zero error */
    if (arg_right != 0) {
...
static void op_mul_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint64_t arg_right = POP();
    TOP() *= arg_right;
...
static void op_jump_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
...
static void op_jump_if_true_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    if (POP()) {
        uint64_t target = code->arg;
        vm_trace.pc = target;
...
static void op_jump_if_false_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    if (!POP()) {
        uint64_t target = code->arg;
        vm_trace.pc =  target;
...
static void op_equal_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint64_t arg_right = POP();
    TOP() = TOP() == arg_right;
...
static void op_less_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint64_t arg_right = POP();
    TOP() = TOP() < arg_right;
...
static void op_less_or_equal_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint64_t arg_right = POP();
    TOP() = TOP() <= arg_right;
...
static void op_greater_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint64_t arg_right = POP();
    TOP() = TOP() > arg_right;
...
static void op_greater_or_equal_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint64_t arg_right = POP();
    TOP() = TOP() >= arg_right;
...
static void op_greater_or_equali_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
...
static void op_pop_res_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint64_t res = POP();
    vm_trace.result = res;
...
static void op_done_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    (void) code;
...
static void op_print_handler(scode *code, uint64_t *stack_top, uint64_t acc)
{
    uint64_t arg = POP();
    printf("%" PRIu64 "\n", arg);
...
static void trace_tail_handler(scode *code, uint64_t* stack_top, uint64_t acc)
{
...
static void trace_prejump_handler(scode *code, uint64_t* stack_top, uint64_t acc)
{
    vm_trace.pc = code->arg;
...
static void trace_compile_handler(scode *trace_head, uint64_t *stack_top, uint64_t acc)
{
    ...
    /* now, run the chain */
    trace_head->handler(trace_head, stack_top, acc);
}
...
interpret_result vm_interpret_trace(uint8_t *bytecode)
{
    ...
    if (!setjmp(vm_trace.buf)) {
        while(vm_trace.is_running) {
            ...
            code->handler(code, vm_trace.stack_top, vm_trace.acc);
        }
    }
